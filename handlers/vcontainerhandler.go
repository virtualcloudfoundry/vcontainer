...
func (v *vcontainerHandler) CurrentMemoryLimit(ctx context.Context, in *google_protobuf.Empty) (*vcontainermodels.MemoryLimits, error) {
	containerId, err := v.getContainerId(ctx)
	if err != nil {
		v.logger.Error("vcontainer-bulk-net-out-get-container-id-failed", err)
		return nil, err
	...
	if err != nil {
		...
		return &vcontainermodels.MemoryLimits{
			LimitInBytes: uint64(containerGroup.Containers[0].ContainerProperties.Resources.Limits.MemoryInGB * 1024 * 1024 * 1024),
		}, nil
	}
}

func (v *vcontainerHandler) CurrentCPULimit(ctx context.Context, in *google_protobuf.Empty) (*vcontainermodels.CPULimits, error) {
	containerId, err := v.getContainerId(ctx)
	if err != nil {
		v.logger.Error("vcontainer-bulk-net-out-get-container-id-failed", err)
		return nil, err
	...
	if err != nil {
		...
		return &vcontainermodels.CPULimits{
			LimitInShares: uint64(containerGroup.Containers[0].ContainerProperties.Resources.Limits.CPU),
		}, nil
	}
}
...
func (v *vcontainerHandler) StreamOut(outSpec *vcontainermodels.StreamOutSpec, server vcontainermodels.VContainer_StreamOutServer) error {
	...
	for {
		...
		if err != nil {
			if err != io.EOF {
				v.logger.Error("vcontainer-stream-out-read-failed", err, lager.Data{"n": n, "err": err})
				return verrors.New("vcontainer-stream-out-failed")
			} else {
				v.logger.Info("vcontainer-stream-out-read-eof-got", lager.Data{"file_id": fileId})
				break
			}
		}
...
func (v *vcontainerHandler) Info(ctx context.Context, empty *google_protobuf.Empty) (*vcontainermodels.ContainerInfo, error) {
	...
	if containerGroup.IPAddress != nil {
		...
		containerInfo.ExternalIP = containerGroup.IPAddress.IP
