...
func (v *vprocessHandler) Signal(ctx context.Context, req *vcontainermodels.SignalRequest) (*google_protobuf.Empty, error) {
	...
	if err != nil {
		v.logger.Error("vprocess-signal-get-task-id-failed", err)
		return nil, verrors.New("failed to get process id.")
	}

	// find the process id from the task id.
	pid, err := containerInterop.GetProcessIdFromTaskId(taskId)
	if err != nil {
		v.logger.Error("vprocess-signal-get-pid-failed", err, lager.Data{"task_id": taskId})
		return nil, err
	} else {
		killCmd := interop.RunTask{
			User: "",
			Path: "kill",
			Args: []string{"-s", strconv.Itoa(int(req.Signal)), strconv.Itoa(int(pid))},
			Tag:  "kill",
		}
	...
	for {
		...
		if waitResponse.Exited {
			v.logger.Info("vprocess-wait-process-exited", lager.Data{
				"wait_exit_code": waitResponse.ExitCode,
				"container_id":   containerId,
				"task_id":        taskId})
			break
		} else {
...
func (v *vprocessHandler) SetTTY(ctx context.Context, spec *vcontainermodels.TTYSpec) (*google_protobuf.Empty, error) {
	v.logger.Info("vprocess-set-tty", lager.Data{"spec": spec})
	return nil, nil
}
